...
- **Barang Masuk**: Purchase Order → Receive Barang → Scan IMEI saat terima → Cetak barcode.
- **Stock**, dipecah per dimensi:
  - Per Gudang, Per Rak, Per Marketplace
  - Per status: Reserved · Available · Damaged · Lost
  - Satu unit fisik (via IMEI) hanya boleh berada di satu kombinasi
...
## 4. IMEI Management
Bukan kolom, tapi **entitas dengan histori penuh**:
```
IMEI → Supplier → Tanggal Masuk → Gudang → Marketplace → Order → Customer → Garansi → Retur
```
Scan satu IMEI = seluruh riwayat langsung muncul (dipakai lintas modul: Inventory, POS,
...
scan berturut-turut). Scan barcode/IMEI · Diskon · Voucher · Member · QRIS/Cash/Transfer ·
Split Payment · Print Thermal · Invoice PDF.

## 8. Packing Station
Halaman khusus admin gudang, **custom Livewire**, alur linear dan ketat:
```
Scan Invoice → Scan IMEI → Validasi → Print Resi → Packing → Done
```
Kalau IMEI yang di-scan tidak cocok dengan yang seharusnya ada di order tersebut → **tidak
...
ditelusuri lewat IMEI, bukan lewat nomor order saja.
...
- **Label Barcode & QR Code** — cetak label produk dan IMEI.
