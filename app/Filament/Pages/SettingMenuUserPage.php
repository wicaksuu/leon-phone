<?php

namespace App\Filament\Pages;

use BackedEnum;
use Filament\Pages\Page;

/**
 * Placeholder navigasi (docs/00-status.md #17) — isi sebenarnya dibangun
 * sesuai urutan docs/07-roadmap.md, bukan sekarang. Halaman ini hanya
 * memastikan struktur sidebar sudah sesuai referensi (ref-gambar/) sejak
 * awal.
 */
class SettingMenuUserPage extends Page
{
    protected string $view = 'filament.pages.setting-menu-user';

    protected static ?string $slug = 'setting-menu-user';

    protected static string|\UnitEnum|null $navigationGroup = 'Utiliti';

    protected static string|BackedEnum|null $navigationIcon = 'heroicon-o-bars-3';

    protected static ?string $navigationLabel = 'Setting Menu User';

    protected static ?int $navigationSort = 210;

    protected static ?string $title = 'Setting Menu User';
}
